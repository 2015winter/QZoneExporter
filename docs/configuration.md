...
| 下载方式 | 推荐度 | 适用场景 | 优缺点 |
|---------|-------|---------|--------|
| **Aria2** | ⭐⭐⭐⭐⭐ | 文件较多 | ✅ 功能强大<br>✅ 支持队列控制<br>✅ 支持断点续传<br>❌ 需要安装配置 |
| **Motrix** | ⭐⭐⭐⭐ | 文件较多 | ✅ 图形界面易用<br>✅ 内置 Aria2<br>❌ 需要安装 |
| **浏览器** | ⭐⭐⭐ | 文件较少 | ✅ 无需额外软件<br>❌ 大量下载易崩溃<br>❌ 视频下载失败率高 |
| **迅雷** | ⭐⭐ | 习惯迅雷 | ✅ 下载速度快<br>❌ V3版需手动复制链接<br>❌ 无法自动唤起 |
...
Aria2 是推荐的下载方式，v3.0 版本新增了队列控制功能：

| 配置项 | 默认值 | 说明 |
|--------|-------|------|
| RPC 地址 | `http://localhost:16800/jsonrpc` | Aria2 RPC 服务地址 |
| RPC 密钥 | （无） | Aria2 RPC 密钥（如有） |
| 最大等待任务数 | 100 | 当活跃+等待任务数超过此值时暂停添加 |
| 队列检查间隔 | 3秒 | 检查队列状态的时间间隔 |
| 启用队列控制 | ✅ 开启 | 防止任务堆积过多 |
| 自动清理卡住任务 | ✅ 开启 | 自动处理下载卡住的任务 |
| 卡住检查间隔 | 5批 | 每添加多少批任务检查一次 |
...
> ⚠️ V3 版本迅雷下载为链接复制模式，以下配置用于生成下载链接列表：

| 配置项 | 默认值 | 说明 |
|--------|-------|------|
| 最大任务数 | 1500 | 每次生成的链接数量上限 |
| 生成间隔 | 60秒 | 分批生成链接的时间间隔 |
...
*本文档基于 v3.0 版本更新，适配 Manifest V3。*
