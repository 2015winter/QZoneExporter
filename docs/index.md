...
| 方式 | 说明 |
|------|------|
| **开发者模式（推荐）** | 下载源码 → 开启开发者模式 → 加载 `src` 目录 |

> ℹ️ 商店里的同名扩展是原作者[芷炫](https://github.com/ShunCai)的官方版（近期已恢复维护并更新到 Manifest V3），与本项目为不同发行。本项目仅以源码 / 开发者模式分发。
...
**备份页浏览体验与稳定性补强**（随下次正式发版纳入）

- **浅色 / 深色**：备份页右下角切换，偏好本地记住
- **顶栏、页脚不再常驻**：导航仅在页首，签名仅在页尾，不再遮挡正文
- **顶栏与页面背景一体**：去掉模块色条，字号与既有备份页一致
- **回到顶部**：长页滚动后可一键返回
- **说说列表铺满内容区**，与日志、相册等模块对齐
- 导出样式按 `theme` / `components` / `pages` 分层，不影响离线打开
- 修复增量备份时评论表情漏下；规范 GET 响应无法解析为 JSON 时的日志
- 已有备份需重新导出（或替换 `Common/`）后生效
