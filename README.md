...
**开发者模式（推荐）** - 下载源码 → 打开 `chrome://extensions/` → 开启开发者模式 → 加载 `src` 目录

> ℹ️ 应用商店中搜到的同名扩展是原作者[芷炫](https://github.com/ShunCai)的官方版（近期已恢复维护并更新到 Manifest V3），与本仓库（社区维护版）为**不同发行**。本项目目前仅以源码 / 开发者模式分发。
